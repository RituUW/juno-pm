...
1. **Problem Summary** — one sentence on the core issue.
2. **Target Persona** — who is suffering most.
3. **Evidence Table** — columns: `Insight | Source ID | Severity (0–1) | Confidence`.
4. **Open Questions** — what we still don't know.
5. **Suggested Next Experiments** — one tactical next step.
...
1. **Identify the Conflict** — where does a stakeholder request (e.g., Sales or CEO) contradict a technical reality (e.g., Engineering)?
2. **Surface the Assumption** — what must be true for this project to succeed that we haven't proven yet?
3. **Evaluate Evidence Strength** — if an insight only comes from one person's opinion, flag it as "Low Confidence" in the Evidence Table.
